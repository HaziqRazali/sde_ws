...
if [ $? != 0 ]; then
  ...
  # ========== sde ==========
  # Sim tf on rviz visualization and metaquest3 teleoperation (with data collection)
  tmux new-window -n sde
  tmux send-keys -t $SESSION_NAME:sde 'ros2 launch sde_simulation sim_main_genesis.launch.py robot_type:=' $ROBOT_TYPE ' sim_env_name:=' $SIM_ENV_NAME ' enable_gui:=true num_ROS2_threads:=8 enable_sim_tf:=true use_sim_time:=' $USE_SIM_TIME C-m
  tmux select-layout -t $SESSION_NAME:sde tiled

  tmux split-window -v -t $SESSION_NAME:sde
  tmux send-keys -t $SESSION_NAME:sde 'sleep 15; ros2 launch casadi_optimal_control_ik robot_ik_solver.launch.py robot_type:=' $ROBOT_TYPE ' sim_or_real:=' $SIM_OR_REAL ' config_file:=ik_solver_config.yaml enable_tf:=false enable_static_world_tf:=true robot_base_link_name:=Trunk use_sim_time:=' $USE_SIM_TIME C-m
  tmux select-layout -t $SESSION_NAME:sde tiled
  ...
  tmux split-window -v -t $SESSION_NAME:sde
  tmux send-keys -t $SESSION_NAME:sde 'ros2 launch ros2bag_server ros2bag_server.launch.py robot_type:=' $ROBOT_TYPE ' config_name:=sim_data_collection_config.yaml path_to_output_dir:=/root/ output_dir:=bag/ is_run_on_robot:=false is_mcap:=false is_override_qos:=false' C-m
  tmux select-layout -t $SESSION_NAME:sde tiled
